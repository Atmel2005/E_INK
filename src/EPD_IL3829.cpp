...
// IL3829 command set (минимально, старт по аналогии SSD1608)
#define IL_SW_RESET         0x12
#define IL_WRITE_RAM_BW     0x24
#define IL_WRITE_RAM_ACC    0x26
#define IL_SET_RAMXPOS      0x44
#define IL_SET_RAMYPOS      0x45
#define IL_SET_RAMXCOUNT    0x4E
#define IL_SET_RAMYCOUNT    0x4F
#define IL_DISPLAY_UPDATE   0x21
#define IL_MASTER_ACT       0x20
#define IL_WRITE_LUT        0x32

static const uint8_t il_lut_full[] = {
  0x02,0x02,0x01,0x11,0x12,0x12,0x22,0x22,
  0x66,0x69,0x69,0x59,0x58,0x99,0x99,0x88,
  0x00,0x00,0x00,0x00,0xF8,0xB4,0x13,0x51,
  0x35,0x51,0x51,0x19,0x01,0x00
};
static const uint8_t il_lut_part[] = {
...
EPD_IL3829::EPD_IL3829(const EPDConfig& cfg) : _cfg(cfg) {
  _phys_w = cfg.width;
  _phys_h = cfg.height;
  _rotation = cfg.rotation;
}

EPD_IL3829::~EPD_IL3829() { end(); }

bool EPD_IL3829::begin() {
  // Unified IO init
  _io.begin();

  hwReset();
  sendCommand(IL_SW_RESET);
  waitBusy();

  setResolution();
  setLUTByHost(true);
  setLUT(_profile == RefreshProfile::Full ? il_lut_full : il_lut_part);
  powerOn();
...
void EPD_IL3829::sleep() {
  // Deep sleep (по даташиту) — пока заглушка
  delay(5);
}

bool EPD_IL3829::isBusy() const {
  return digitalRead(_cfg.busy) == HIGH;
}

void EPD_IL3829::setRotation(uint8_t r) {
  _rotation = r & 3;
  switch (_rotation) {
    case 0:
...
void EPD_IL3829::drawPixel(int16_t x, int16_t y, EPDColor color) {
  ...
  switch (color) {
    ...
    case EPDColor::White:
    default:                pixClr(_fb_black, xp, yp); if (_cfg.variant != EPDVariant::BW) pixClr(_fb_accent, xp, yp); break;
  }
}

void EPD_IL3829::fillScreen(EPDColor color) {
  if (!_fb_black) return;
...
void EPD_IL3829::flushRect(int16_t x, int16_t y, int16_t w, int16_t h) {
  ...
  for (int16_t yy = ys; yy <= ye; ++yy) {
    sendCommand(IL_SET_RAMXCOUNT); sendData(xs_al >> 3);
    sendCommand(IL_SET_RAMYCOUNT); sendData(yy & 0xFF); sendData((yy >> 8) & 0x01);
    sendCommand(IL_WRITE_RAM_BW);
...
void EPD_IL3829::hwReset() {
  if (_cfg.rst < 0) return;
  digitalWrite(_cfg.rst, LOW); delay(10);
  digitalWrite(_cfg.rst, HIGH); delay(10);
  waitBusy();
}
void EPD_IL3829::waitBusy() {
  while (isBusy()) delay(1);
}
void EPD_IL3829::sendCommand(uint8_t c) { _io.sendCommand(c); }
void EPD_IL3829::sendData(uint8_t d) { _io.sendData(d); }
void EPD_IL3829::sendDataBlock(const uint8_t* p, size_t n) { _io.sendDataBlock(p, n); }
void EPD_IL3829::setWindow(int16_t x0, int16_t y0, int16_t x1, int16_t y1) {
  sendCommand(IL_SET_RAMXPOS); sendData(x0 >> 3); sendData(x1 >> 3);
  sendCommand(IL_SET_RAMYPOS);
  sendData(y0 & 0xFF); sendData((y0 >> 8) & 0x01);
  sendData(y1 & 0xFF); sendData((y1 >> 8) & 0x01);
  sendCommand(IL_SET_RAMXCOUNT); sendData(x0 >> 3);
  sendCommand(IL_SET_RAMYCOUNT);
  sendData(y0 & 0xFF); sendData((y0 >> 8) & 0x01);
}
void EPD_IL3829::setCursor(int16_t, int16_t) {}
void EPD_IL3829::issueUpdate(RefreshProfile p) {
  sendCommand(IL_DISPLAY_UPDATE);
  sendData(p == RefreshProfile::Full ? 0xC7 : 0x0F);
  sendCommand(IL_MASTER_ACT);
  waitBusy();
}
void EPD_IL3829::allocateBuffer() {
  freeBuffer();
...
void EPD_IL3829::mapXY(int16_t x, int16_t y, int16_t& xp, int16_t& yp) const {
  switch (_rotation) {
...
void EPD_IL3829::mapRectToPhys(int16_t x, int16_t y, int16_t w, int16_t h,
...
void EPD_IL3829::setLUT(const uint8_t* lut) {
  sendCommand(IL_WRITE_LUT);
  for (int i=0;i<30;i++) sendData(lut[i]);
}
void EPD_IL3829::powerOn() {
  sendCommand(0x22); sendData(0xC0); sendCommand(0x20); waitBusy();
}
void EPD_IL3829::powerOff() {
  sendCommand(0x22); sendData(0x83); sendCommand(0x20); waitBusy();
}
void EPD_IL3829::setResolution() {
  sendCommand(0x61);
  sendData(_phys_w);
  sendData((_phys_h >> 8) & 0x01);
  sendData(_phys_h & 0xFF);
}
void EPD_IL3829::setLUTByHost(bool enabled) {
  sendCommand(0x22);
  sendData(enabled ? 0x40 : 0x00);
  sendCommand(0x20);
  waitBusy();
}
#endif // EINK_SELECTIVE_BUILD / EINK_ENABLE_IL3829
